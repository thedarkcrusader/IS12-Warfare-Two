...
/turf/simulated/open/update_icon()
	...
	if(below)
		...
		if(below_is_open)
			...
			bottom_turf.plane = UNDER_OPENSPACE_PLANE//below.plane + src.plane
			bottom_turf.color = below.color
			bottom_turf.overlays += below.overlays
			underlays += bottom_turf
			for(var/image/I in below.overlays)
				var/image/temp = I
				temp.plane = UNDER_OPENSPACE_PLANE//I.plane + src.plane
				temp.color = I.color
				overlays += temp
		...
		for(var/obj/O in below)
			...
			temp2.plane = UNDER_OPENSPACE_PLANE//O.plane + src.plane
			temp2.color = O.color
...
/turf/simulated/open/attack_hand(mob/living/carbon/human/user as mob)
	...
	if(user.can_fall())//See if they can go down it.
		...
		if(do_after(user, 15))
			if(user.statscheck(user.STAT_LEVEL(dex) / 2 + 3) >= SUCCESS)//Dex check.
