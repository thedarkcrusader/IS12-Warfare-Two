...
/client/proc/drop_bomb() // Some admin dickery that can probably be done better -- TLE
	...
	switch(choice)
		if(null)
			return 0
		if("Small Bomb")
			explosion(epicenter, 1, 2, 3, 3, particles = TRUE)
		if("Medium Bomb")
			explosion(epicenter, 2, 3, 4, 4, particles = TRUE)
		if("Big Bomb")
			explosion(epicenter, 3, 5, 7, 5, particles = TRUE)
		if("Custom Bomb")
			var/devastation_range = input("Devastation range (in tiles):") as num
			var/heavy_impact_range = input("Heavy impact range (in tiles):") as num
			var/light_impact_range = input("Light impact range (in tiles):") as num
			var/flash_range = input("Flash range (in tiles):") as num
			explosion(epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, particles = TRUE)
...
/client/proc/editappear()
	...
	var/new_tone = input("Please select skin tone level: 1-180 (1=albino, 35=caucasian, 150=black)", "Character Generation")  as text

	if (new_tone)
		M.s_tone = max(min(round(text2num(new_tone)), 180), 1)
