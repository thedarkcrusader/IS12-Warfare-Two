...
/mob
	...
	var/recoil = 0
	var/weapon_readied = FALSE //Used for managing recoil
...
/client/MouseDown(object, location, control, params)
	...
	if(delay)
		...
		if(!usr.weapon_readied)
			...
			usr.recoil = 0
		usr.dispersion_mouse_display_number = usr.recoil
...
/mob/proc/ready_weapon()
	if(weapon_readied) //Already readied up. Don't need to do it again.
		return
	weapon_readied = TRUE
...
/mob/proc/unready_weapon()
	if(!weapon_readied)//It's already unreadied
		return
