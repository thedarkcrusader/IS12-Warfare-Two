	...
	for(var/level = 1 to 3)
		for(var/command_position in GLOB.command_positions)
			...
			for(var/mob/V in candidates)
				...
				else if(age - (job.minimum_character_age+10) < ((job.ideal_character_age-10) - (job.minimum_character_age+10) ))
					weightedCandidates[V] = 6
				else if(age - (job.ideal_character_age-10) < ((job.ideal_character_age+10) - (job.ideal_character_age-10)))
					weightedCandidates[V] = 10
				else if(age - (job.ideal_character_age+10) < (job.ideal_character_age+20) - (job.ideal_character_age+10))
